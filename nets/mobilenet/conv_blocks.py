...
def expanded_conv(input_tensor,
  ...
  with tf.variable_scope(scope, default_name='expanded_conv') as s, \
    ...
    input_channels = input_tensor.shape[3].value
    net = input_tensor
    ...
    if se:
      ...
      output_num = net_se.shape[3].value

      if se_channel_num:
        num_reduce_filters = se_channel_num
      elif eff_mode:
        num_reduce_filters = max(1, int(channel_num*se_ratio))
      else:
        num_reduce_filters = max(1, int(output_num*se_ratio))

      net_se = tf.reduce_mean(net_se, [1, 2], keep_dims=True)
      net_se = slim.conv2d(net_se, num_reduce_filters, [1,1], scope="se_1", activation_fn=my_swish, normalizer_fn=None)
      net_se = slim.conv2d(net_se, output_num, [1,1], scope="se_2", activation_fn=tf.nn.sigmoid, normalizer_fn=None)
      net = net*net_se
